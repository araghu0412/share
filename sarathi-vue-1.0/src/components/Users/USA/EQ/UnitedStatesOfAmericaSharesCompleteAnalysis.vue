<template>
  <div class="united-states-of-america-shares-complete-analysis">
...
export default {
  ...
  methods: {
    getAnalysisDetails() {
      ...
      if(this.$store.getters["unitedStatesOfAmericaSharesCompleteAnalysis/GET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_MAP"].size === 0) {
        axios.get(services.unitedStatesOfAmericaSharesCompleteAnalysis).then(response => {
          if (response.status === 204) {
            ...
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_MAP", response.data.unitedStatesOfAmericaSharesSectorMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_LONG_TERM_ONLY_MAP", response.data.unitedStatesOfAmericaSharesSectorLongTermOnlyMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_CATEGORY_MAP", response.data.unitedStatesOfAmericaSharesCategoryMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_CATEGORY_LONG_TERM_ONLY_MAP", response.data.unitedStatesOfAmericaSharesCategoryLongTermOnlyMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_SECTOR_DETAILS_MAP", response.data.unitedStatesOfAmericaSharesOneSectorDetailsMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_SECTOR_DETAILS_LONG_TERM_ONLY_MAP", response.data.unitedStatesOfAmericaSharesOneSectorDetailsLongTermOnlyMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_CATEGORY_DETAILS_MAP", response.data.unitedStatesOfAmericaSharesOneCategoryDetailsMap);
            this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_CATEGORY_DETAILS_LONG_TERM_ONLY_MAP", response.data.unitedStatesOfAmericaSharesOneCategoryDetailsLongTermOnlyMap);
    ...
    getAnalysisData() {
      ...
      if (this.isSectorAnalysis && !this.isLongTermOnly) {
        for (let [key, val] of this.$store.getters["unitedStatesOfAmericaSharesCompleteAnalysis/GET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_MAP"]) {
          if (key !== "total") {
        ...
        for (let [key, val] of this.$store.getters["unitedStatesOfAmericaSharesCompleteAnalysis/GET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_LONG_TERM_ONLY_MAP"]) {
          if (key !== "total") {
            this.oneAnalysisDropdownList.push({ id: val.sectorName, value: val.sectorName});
            xAxisTitlesList.push(val.sectorName);
            if (!this.isValueInPercentage) {
        ...
        for (let [key, val] of this.$store.getters["unitedStatesOfAmericaSharesCompleteAnalysis/GET_UNITED_STATES_OF_AMERICA_SHARES_CATEGORY_MAP"]) {
          if (key !== "total") {
        ...
        for (let [key, val] of this.$store.getters["unitedStatesOfAmericaSharesCompleteAnalysis/GET_UNITED_STATES_OF_AMERICA_SHARES_CATEGORY_LONG_TERM_ONLY_MAP"]) {
          if (key !== "total") {
            this.oneAnalysisDropdownList.push({ id: val.categoryName, value: val.categoryName});
            xAxisTitlesList.push(val.categoryName);
            if (!this.isValueInPercentage) {
    ...
    onRefreshClick() {
      // Clear store
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_SECTOR_LONG_TERM_ONLY_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_CATEGORY_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_CATEGORY_LONG_TERM_ONLY_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_SECTOR_DETAILS_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_SECTOR_DETAILS_LONG_TERM_ONLY_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_CATEGORY_DETAILS_MAP", new Map());
      this.$store.commit("unitedStatesOfAmericaSharesCompleteAnalysis/SET_UNITED_STATES_OF_AMERICA_SHARES_ONE_CATEGORY_DETAILS_LONG_TERM_ONLY_MAP", new Map());
    ...
    onOneAnalysisDropdownSelect(selectedType) {
      ...
      this.$router.push("/users/USA/EQ/one-analysis?type=" + type + "&name=" + selectedType.value + "&isLongTermOnly=" + isLongTermOnly + "&isValueInPercentage=" + isValueInPercentage + "&analysisService=complete-analysis");
    },
