...
@Component({
  selector: 'app-united-states-of-america-shares-dividend-yield-analysis',
  templateUrl: './united-states-of-america-shares-dividend-yield-analysis.component.html',
  styleUrls: ['./united-states-of-america-shares-dividend-yield-analysis.component.css']
})
export class UnitedStatesOfAmericaSharesDividendYieldAnalysisComponent implements OnInit {
  ...
  getAnalysisDetails() {
    ...
    if(this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.getUnitedStatesOfAmericaSharesSectorMap().size === 0) {
      this.http.get(services.unitedStatesOfAmericaSharesDividendYieldAnalysis).subscribe((response: any) => {
        if (response.status === 204) {
          ...
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesSectorMap(response.unitedStatesOfAmericaSharesSectorMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesSectorLongTermOnlyMap(response.unitedStatesOfAmericaSharesSectorLongTermOnlyMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesCategoryMap(response.unitedStatesOfAmericaSharesCategoryMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesCategoryLongTermOnlyMap(response.unitedStatesOfAmericaSharesCategoryLongTermOnlyMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesOneSectorDetailsMap(response.unitedStatesOfAmericaSharesOneSectorDetailsMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesOneSectorDetailsLongTermOnlyMap(response.unitedStatesOfAmericaSharesOneSectorDetailsLongTermOnlyMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesOneCategoryDetailsMap(response.unitedStatesOfAmericaSharesOneCategoryDetailsMap);
          this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.setUnitedStatesOfAmericaSharesOneCategoryDetailsLongTermOnlyMap(response.unitedStatesOfAmericaSharesOneCategoryDetailsLongTermOnlyMap);
  ...
  getAnalysisData() {
    ...
    if (this.isSectorAnalysis && !this.isLongTermOnly) {
      for (let [key, val] of this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.getUnitedStatesOfAmericaSharesSectorMap()) {
        if (key !== "total") {
      ...
      for (let [key, val] of this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.getUnitedStatesOfAmericaSharesSectorLongTermOnlyMap()) {
        if (key !== "total") {
          this.oneAnalysisDropdownList.push({ id: val.sectorName, value: val.sectorName});
          xAxisTitlesList.push(val.sectorName);
          if (!this.isValueInPercentage) {
      ...
      for (let [key, val] of this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.getUnitedStatesOfAmericaSharesCategoryMap()) {
        if (key !== "total") {
      ...
      for (let [key, val] of this.unitedStatesOfAmericaSharesDividendYieldAnalysisStoreService.getUnitedStatesOfAmericaSharesCategoryLongTermOnlyMap()) {
        if (key !== "total") {
          this.oneAnalysisDropdownList.push({ id: val.categoryName, value: val.categoryName});
          xAxisTitlesList.push(val.categoryName);
          if (!this.isValueInPercentage) {
  ...
  onOneAnalysisDropdownSelect(selectedType: any) {
    ...
    this.router.navigate(["/users/USA/EQ/one-analysis"], { queryParams: { type: type,  name: selectedType.value, isLongTermOnly: isLongTermOnly, isValueInPercentage: isValueInPercentage, analysisService: "dividend-yield-analysis" } });
  }
