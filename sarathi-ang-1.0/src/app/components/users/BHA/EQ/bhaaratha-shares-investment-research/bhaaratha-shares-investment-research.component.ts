...
import { BhaarathaSharesInvestmentResearchStoreService } from 'src/app/common/store/bhaaratha-shares-investment-research/bhaaratha-shares-investment-research-store.service';
import services from 'src/assets/services/services.json';

@Component({
  selector: 'app-bhaaratha-shares-investment-research',
  templateUrl: './bhaaratha-shares-investment-research.component.html',
  styleUrls: ['./bhaaratha-shares-investment-research.component.css']
})
export class BhaarathaSharesInvestmentResearchComponent implements OnInit {
  ...
  getInvestmentResearch() {
    ...
    if (this.bhaarathaSharesInvestmentResearchStoreService.getBhaarathaSharesInvestmentResearchList().length === 0) {
      this.http.get(services.bhaarathaSharesInvestmentResearch).subscribe((response:any) => {
        if (response.status === 204) {
          this.noRecords = true;
          this.loading = false;
          this.isRefreshButtonDisabled = false;
        } else {
          this.bhaarathaSharesInvestmentResearchStoreService.setBhaarathaSharesInvestmentResearchList(response.bhaarathaSharesInvestmentResearchList);
          this.noRecords = false;
          this.loading = false;
          this.isRefreshButtonDisabled = false;
        }

        this.defaultInvestmentResearchList = this.bhaarathaSharesInvestmentResearchStoreService.getBhaarathaSharesInvestmentResearchList();
